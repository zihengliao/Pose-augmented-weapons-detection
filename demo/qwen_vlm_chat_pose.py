...
def load_qwen25vlm_model(model_path):
    """Load Qwen2.5-VLM model with correct approach"""
    print("Loading Qwen2.5-VLM model...")
    ...
    try:
        ...
        print("Model loaded with Qwen2_5_VLForConditionalGeneration")
        return model, processor, "qwen2_5vl"
...
def run_inference_qwen2vl(model, processor, image_obj, prompt,
                          ...
                          top_p=1.0,
                          top_k=0):
    """Run inference with Qwen2.5-VL. image_obj is a PIL.Image."""
    from qwen_vl_utils import process_vision_info  # keep import near usage

    chat_text = make_chat_text(prompt)

    # Pack inputs (text + 1 image)
    inputs = processor(
        text=[chat_text],
        images=[image_obj],
        return_tensors="pt",
...
def main():
    # Paths
    MODEL_PATH = "/fs04/scratch2/ml23/zlia0050/fine_tune/COMPLETELY_NEW_FROM_SCRATCH/pose-augmented-weapon-detection-using-machine-learning/output-bbox-caption"
    ...
    prompt = "give me the bboxes of the 2 guns you see in this image in coco formats"
    print(f"\n{'='*70}")
    print("QWEN2.5-VLM WEAPON DETECTION TEST")
    print(f"{'='*70}")
    ...
    try:
        while True:
            ...
            if user_in.lower() == "/greedy":
                do_sample = False
                print("Decoding set to GREEDY (deterministic).")
                continue
            if user_in.lower() == "/sample":
                do_sample = True
                print("Decoding set to SAMPLING (stochastic).")
                continue
