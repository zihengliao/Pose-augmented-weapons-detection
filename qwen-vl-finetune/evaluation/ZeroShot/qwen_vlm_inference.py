# -*- coding: utf-8 -*-
"""
Qwen VLM Zero-Shot Inference Script

This script performs zero-shot object detection using Qwen2.5-VL models
with optional PEFT adapter support and COCO evaluation metrics.
"""
...
def print(*args, **kwargs):
    ...
    try:
        ...
        text = ""
    auto_critical = ("Error" in text) or ("Warning" in text) or ("CRITICAL" in text)
...
DEFAULT_ADAPTER_PATH: Optional[str] = None
...
REQUEST_TOP_K_DETECTIONS = 100
DETECTION_MAX_NEW_TOKENS = 8192

# Evaluation behavior
COLLAPSE_CLASSES_TO_WEAPON = True
COLLAPSED_CLASS_NAME = "gun"

# Cache configuration
HF_CACHE_DIR = os.getenv("HF_CACHE_DIR", None)
ROBOFLOW_CACHE_DIR = os.getenv("ROBOFLOW_CACHE_DIR", "./roboflow_data")
...
def _initialize_model_paths():
    """Initialize model and adapter paths from command-line arguments or environment."""
    ...
    try:
        ...
        parser.add_argument("--adapter_path", type=str, help="Optional PEFT adapter path")
...
class QwenVLMInference:
            ...
            try:
                ...
                try:
                    ...
                    if merged_model is not None:
                        ...
                        print("Adapter merged into base model weights.", level=2)
                    else:
                        print("Info: Adapter kept in PEFT mode (merge returned None).", level=2)
                except AttributeError:
                    print("Info: merge_and_unload not available; using adapter in PEFT mode.", level=2)
    ...
    def _warmup(self):
        ...
        try:
            ...
            print(f"Model warmup completed with warning: {e}", level=2)
    ...
    def detect_objects(self, image_path: str, class_names: List[str],
        ...
        prompt = f"""You are an object detection system. Analyze this image and detect objects from these classes: {class_list}

CRITICAL: Return ONLY a JSON array. NO explanations, NO reasoning, NO additional text.

Format: [{{"class": "class_name", "confidence": 0.95, "bbox": [x1, y1, x2, y2]}}]

IMPORTANT REQUIREMENTS:
- bbox coordinates MUST be normalized between 0.0 and 1.0
...
- Return as many detections as possible up to {REQUEST_TOP_K_DETECTIONS} items, ordered from highest to lowest confidence
- Return empty array [] if no objects detected
- Output ONLY the JSON array, nothing else

Example: [{{"class": "gun", "confidence": 0.87, "bbox": [0.12, 0.34, 0.67, 0.89]}}]
...
JSON array:"""
    ...
    def _parse_detections(self, output_text: str, confidence_threshold: float, img_width: int, img_height: int) -> List[Dict]:
        ...
        detections = []
        print(f"Parsing model output (first 200 chars): {output_text[:200]}...", level=2)
                            ...
                            if (isinstance(detection, dict) and
                                ...
                                if (confidence >= confidence_threshold and
                                    ...
                                    if any(val > 1.0 for val in bbox_values):
                                        print(f"Warning: Detected pixel coordinates: {bbox_values}. Converting to normalized coordinates.", level=2)
                                    ...
                                    detections.append({
                                        'class': str(detection['class']).strip().lower(),
                                        'confidence': confidence,
                                        'bbox': bbox_clean
                                    })

                except json.JSONDecodeError as je:
                    print(f"Warning: JSON decode error: {je}. Using strict COCO evaluation standards.", level=2)
                    print(f"Problematic JSON: {json_str}", level=2)
            else:
                print("Warning: No JSON pattern found in model output. Using strict evaluation standards.", level=2)
...
class RoboflowDataset:
    ...
    def download_dataset(self, workspace: str, project: str, version: int) -> str:
        ...
        try:
            ...
            dataset = project_obj.version(version).download("coco", location=ROBOFLOW_CACHE_DIR)
...
def main():
    ...
    print(f"   Roboflow cache: {ROBOFLOW_CACHE_DIR}", level=1)
    if ADAPTER_PATH:
    ...
    for i, (image_path, gt_annotations) in enumerate(_iterable):
        try:
            ...
            # Force single-class predictions: map all predicted classes to 'gun'
